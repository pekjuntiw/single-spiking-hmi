...
from dataset import TTFSFingerprint, loader
...
def main():
    ...
    FLAGS = {
        "dataset_dir": 'FVC2002DB2/generated_40x50_invert_no_repetition_15deg_10000',
        ...
        "train_size": 8000,
        "lr": 1e-2,
        "decay_step": 125,  # 375,  # 50,
        "decay_rate": 0.9,
        "loss_mode": 'mse',
        "weight_bump": True,
        "weight_bump_param": {
            'max_no_spike': [0.45, 0.15],
        ...
        "input_bias_times": None,  #[[25e-6, 35e-6, 45e-6]],  # 2D list of input bias spike times (1st dim units, 2nd dim times), or None
        ...
        "num_in": 2000,  # exclude bias neurons
        "num_hidden": [350],  # exclude bias neurons
        "num_out": 5,
        "weight_init": {"mean": [0.02, 0.15], "std": [0.8, 0.8]},
        "l2_reg": 5,
    ...
    dataset_dir = 'FVC2002DB1/generated' if "dataset_dir" not in FLAGS.keys() else FLAGS["dataset_dir"]
    print(f'Using dataset from {dataset_dir}')
    model_num = 6
    model_dir = f'ttfs_fvc_v2_ideal/{model_num}'
    model_output_dir = f'{model_dir}/model'
    log_dir = f'{model_dir}/log'
    fig_dir = f'{model_dir}/fig'
    model_output_name = f'{model_output_dir}/ttfs_fvc_v2_ideal'  # to be completed below

    # set if resume training from a saved model
    resume_training = False
    resume_from_epoch = 199
    ...
    noise_std = 0.0 if "noise_std" not in FLAGS.keys() else FLAGS["noise_std"]

    writer = SummaryWriter(log_dir)

    # initialize dataloader
    dataset = TTFSFingerprint(
        path=dataset_dir, dt=dt,
        input_bias_units=input_bias_units, input_bias_times=input_bias_times, invert=False,  # True,
        transform=partial(utils.apply_noise, mean=0., std=noise_std, scale=tau)
    )
    ...
    train_dataset, test_dataset = data.random_split(
        dataset, [train_size, len(dataset) - train_size], generator=torch.Generator().manual_seed(20241026)
    )
    ...
    for epoch in range(start_epoch, train_epoch):
        ...
        for i, (fingerprint, label) in enumerate(
            ...
            if i % 20 == 0:
                ...
                for cl in range(num_out):
                    ax[cl // 3, cl % 3].plot(np.arange(train_times) / train_batch_per_epoch, fc2_step[cl])
